...
class BasePage {
	 ...
	 //Exception Handling: indicating that it can throw this specific checked exception, which requires callers to handle the exception or declare it further up the call stack.
	 private Map<String, String> extractQueryParameters(String url) throws URISyntaxException {
		 	//The Map and HashMap use generics (<String, String>), allowing type-safe storage of key-value pairs, which enhances code readability and reduces runtime errors.
	        ...
	        if (query != null) {
	        	//String manipulation:split(), and array indexing to manipulate and process strings
